...
test("toggles and persists the primary sidebar from the button and keyboard shortcut", async () => {
  ...
  try {
    ...
    const settingsBox = await window.locator(".sidebar__footer").getByRole("button", { name: "Settings", exact: true }).boundingBox();
    ...
    const settingsRight = (settingsBox?.x ?? 0) + (settingsBox?.width ?? 0);
    ...
    expect(toggleBox?.x ?? 0).toBeGreaterThanOrEqual(settingsRight + 6);
